...
function _checkCharData(oform,flag,name,disp){
	...
	if(flag=="1"){
		if(obj.value==""){
			alert(disp+aibi_core_messageBundle.alert_notEmpty);
			obj.focus();
			return false;
		}
	}
	...
	if (data.indexOf("'") >= 0)  {
		alert(disp+aibi_core_messageBundle.alert_notInput+"'");
		obj.focus();
		return false;
	}
        ...
        if(r!=null )
        {
		alert(disp+aibi_core_messageBundle.alert_notValidChar);
		obj.focus();
		return false;
	}
...
function _checkIntData(oform,flag,name,disp){
    ...
    r=data.match(re);
    if(r!=null){
		alert(disp+aibi_core_messageBundle.alert_onlyInt);
		obj.focus();
		return false;
	}
	if(data.length>1 && data.substr(0,1)=='0'){
		alert(disp+aibi_core_messageBundle.alert_onlyInteger);
		obj.focus();
		return false;
	}
...
function _checkFloatData(oform,flag,name,disp){
	...
	var r,re;
        ...
        r=data.match(re);
        if(r!=null){
		alert(disp+aibi_core_messageBundle.alert_onlyFloat);
		obj.focus();
		return false;
	}
	...
	if(data.length>1){
		if(data.substring(0,1)=="0" && data.substring(1,2)!="."){
			alert(disp+aibi_core_messageBundle.alert_notValid);
			return false;
		}
		}
	if(data.indexOf(".")>0 &&(data.length-(data.indexOf(".")))>3){
		alert(disp+aibi_core_messageBundle.alert_noMoreThan2Decimal);
		return false;
	}
	return true;
}
...
function _checkMaxLength(oform,name,maxLen,disp) {
	...
	if (checkStrLen(obj.value) > maxLen) {
		alert(disp + aibi_core_messageBundle.alert_lengthLimit128);
		obj.focus();
		return false;
	}
	return true;	
...
function SpecialString(string,compare,BelongOrNot)
{
	...
	{
		alert("function SpecialString(string,compare,BelongOrNot)"+aibi_core_messageBundle.alert_paramError);
		return false;
	}
